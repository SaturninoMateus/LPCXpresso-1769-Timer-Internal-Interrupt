 ...
 *
 *****************************************************************************/
static void hLine(uint8_t x0, uint8_t y0, uint8_t x1, display_color_t color)
{
    // TODO: optimize
 ...
 *
 *****************************************************************************/
static void vLine(uint8_t x0, uint8_t y0, uint8_t y1, display_color_t color)
{
    uint8_t bak;
...
void display_new(){
	...
	display_clearScreen(DISPLAY_COLOR_WHITE);

	display_putString(1,1,  (uint8_t*)"LUZ:   ", DISPLAY_COLOR_BLACK, DISPLAY_COLOR_WHITE);


}
void display_write(uint32_t lReceivedValue){
	intToString(lReceivedValue, buf, 10, 10);
	display_fillRect((1+6*6),1, 80, 8, DISPLAY_COLOR_WHITE);
	display_putString((1+6*6),1, buf, DISPLAY_COLOR_BLACK, DISPLAY_COLOR_WHITE);
}
...
void display_circle(uint8_t x0, uint8_t y0, uint8_t r, display_color_t color)
{
    ...
    {
        display_putPixel(draw_x0, draw_y0, color);     /* 90 degree */
    }
    ...
    if(draw_y2 >= 0)
    {
        display_putPixel(draw_x2, draw_y2, color);    /* 270 degree */
    }
    ...
    {
        display_putPixel(draw_x4, draw_y4, color);     /* 0 degree */
    }
    ...
    if(draw_x5>=0)
    {
        display_putPixel(draw_x5, draw_y5, color);     /* 180 degree */
    }
    ...
    while(xx < yy)
    {
        ...
        {
            display_putPixel(draw_x0, draw_y0, color);
        }

        if( (draw_x1 >= 0) && (draw_y1 >= 0) )
        {
            display_putPixel(draw_x1, draw_y1, color);
        }

        if( (draw_x2 <= OLED_DISPLAY_WIDTH) && (draw_y2 <= OLED_DISPLAY_HEIGHT) )
        {
            display_putPixel(draw_x2, draw_y2, color);
        }

        if( (draw_x3 >=0 ) && (draw_y3 <= OLED_DISPLAY_HEIGHT) )
        {
            display_putPixel(draw_x3, draw_y3, color);
        }

        if( (draw_x4 <= /*OLED_DISPLAY_HEIGHT*/OLED_DISPLAY_WIDTH) && (draw_y4 >= 0) )
        {
            display_putPixel(draw_x4, draw_y4, color);
        }

        if( (draw_x5 >= 0) && (draw_y5 >= 0) )
        {
            display_putPixel(draw_x5, draw_y5, color);
        }
        if( (draw_x6 <= OLED_DISPLAY_WIDTH) && (draw_y6 <= OLED_DISPLAY_HEIGHT) )
        {
            display_putPixel(draw_x6, draw_y6, color);
        }
        if( (draw_x7 >= 0) && (draw_y7 <= OLED_DISPLAY_HEIGHT) )
        {
            display_putPixel(draw_x7, draw_y7, color);
        }
    }
    return;
}
...
uint8_t display_putChar(uint8_t x, uint8_t y, uint8_t ch, display_color_t fb, display_color_t bg)
{
    ...
    display_color_t color = DISPLAY_COLOR_BLACK;
        ...
        display_color_t bg)
{
  while(1)
  {
      ...
      if( display_putChar(x, y, *pStr++, fb, bg) == 0 )
      {
        break;
    }
    x += 6;
